...
class UClimbAbilityComponent;
class ULandAssistAbilityComponent;
...
public:
	...
	*/
    ...
    float GetMaxWalkSpeed() const { return fMaxWalkSpeed; }
    void OnLandAssistStart();
protected:
...
//////////////////////////////////////////////////////////////////////////
	// ABOVE UNREAL SHOOTER GAME CODE 
	//////////////////////////////////////////////////////////////////////////
	// BELOW CUSTOM CODE FOR THE GAME
//////////////////////////////////////////////////////////////////////////
public:
  
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Pickup_Dash")
    float fJumpVelocityAfterDashPickpu = 600.f;

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "JumpAssist")
    float fJumpAssistTime = 0.25f;

    bool CanDash() const;
    void ResetCanDash(float no_gravity_duration);
    //bool IsDashing() const { return bWantsToDash; }
    float GetTimeInAir() const { return fTimeInAir; }
private:
    
    //////////////////////////////////////////////////////////////////////////
    // Jump Assist
    void UpdateJumpAssist(float dt);
    int iJumpMaxTimesDefault = -1;
    float fTimeInAir = 0.f;

	void UpdateRunning();
    void UpdateMaxJump();	

    void OnDebugSavePos();
    void OnDebugPastePos();
    static FVector DebugSavePos;


    void LaunchCharacterAfterPickup();
    float fMaxWalkSpeed = -1.f;
    FTimerHandle LaunchCharacterTimer;
...
public:    
    UPROPERTY(VisibleDefaultsOnly, Category = "Ability Components")
    UWallRunComponent* WallRunComponent;    
    UPROPERTY(VisibleDefaultsOnly, Category = "Ability Components")
    UClimbAbilityComponent* ClimbComponent;
    UPROPERTY(VisibleDefaultsOnly, Category = "Ability Components")
    ULandAssistAbilityComponent* LandAssistComponent;
    UPROPERTY(VisibleDefaultsOnly, Category = "Ability Components")
    UDashAbilityComponent* DashComponent;

    /** get wallruning state */
    UFUNCTION(BlueprintCallable, Category = "Wall Runing")
    ...
    FVector2D GetMoveInputLocal() const { return fMoveInputLocal; }
    void OnClimbStart();

private:
    void ClampHorizonatlVelocity();
    FVector2D GetHorizontalVelocity() const;
    void SetHorizontalVelocity(FVector2D velocity);
    FVector2D fMoveInputLocal = FVector2D::ZeroVector;   
};