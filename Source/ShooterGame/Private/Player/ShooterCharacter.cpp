...
FVector AShooterCharacter::DebugSavePos = FVector::ZeroVector;
	...
	: Super(ObjectInitializer.SetDefaultSubobjectClass<UShooterCharacterMovement>(ACharacter::CharacterMovementComponentName))
{
    ...
    ClimbComponent = CreateDefaultSubobject<UClimbAbilityComponent>(TEXT("ClimbComponent"));
    LandAssistComponent = CreateDefaultSubobject<ULandAssistAbilityComponent>(TEXT("LandAssistComponent"));
...
void AShooterCharacter::PostInitializeComponents()
{
    ...
    if (DashComponent)
    {
        DashComponent->PostInitializeComponents();
    }    	
...
void AShooterCharacter::UpdatePawnMeshes()
{
	...
	//gloVA: uncoment it to make him visilae in tpp
...
void AShooterCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
    ...
    //PlayerInputComponent->BindAction("WallRun", IE_Pressed, this, &AShooterCharacter::TryToStartWallRun);
    //PlayerInputComponent->BindAction("WallRun", IE_Released, this, &AShooterCharacter::StopWallRun);
    ...
    PlayerInputComponent->BindAction("DEBUG_SAVE_POS", IE_Pressed, this, &AShooterCharacter::OnDebugSavePos);
    PlayerInputComponent->BindAction("DEBUG_PASTE_POS", IE_Pressed, this, &AShooterCharacter::OnDebugPastePos);

    DashComponent->SetupPlayerInputComponent(PlayerInputComponent);
    WallRunComponent->SetupPlayerInputComponent(PlayerInputComponent);
	//PlayerInputComponent->BindAction("Dash", IE_Released, this, &AShooterCharacter::OnStopDash);

	// running will be always when not shoting or realoading
...
void AShooterCharacter::Tick(float DeltaSeconds)
{
    ...
    if (DebugSavePos.IsZero())
        DebugSavePos = GetActorLocation();
    if (fMaxWalkSpeed < 0.f)
        fMaxWalkSpeed = GetCharacterMovement()->MaxWalkSpeed;
    if (iJumpMaxTimesDefault < 0)
        iJumpMaxTimesDefault = JumpMaxCount;

	/*if (bWantsToRunToggled && !IsRunning())
	{
		SetRunning(false, false);
	}*/    
    ClampHorizonatlVelocity();
	UpdateRunning();
    UpdateMaxJump();    
    UpdateJumpAssist(DeltaSeconds);    
...
void AShooterCharacter::UpdateRunning()
{
	if (bWantsToFire || bIsTargeting)
	{
		OnStopRunning();
	}
...
void AShooterCharacter::UpdateMaxJump()
{
...
void AShooterCharacter::UpdateJumpAssist(float dt)
{
    if (!GetCharacterMovement()->IsFalling())
        fTimeInAir = 0.f;
    else
    ...
    if (fTimeInAir > 0.f && fTimeInAir <= fJumpAssistTime && JumpCurrentCount == 0)
    {
        JumpMaxCount = iJumpMaxTimesDefault + 1; // double jump for a short period
    }
    else
    {
        JumpMaxCount = iJumpMaxTimesDefault;
    }
}

bool AShooterCharacter::CanDash() const
{
    return DashComponent->CanDash();
}

void AShooterCharacter::ResetCanDash(float no_gravity_duration)
{
    DashComponent->ResetCanDash(no_gravity_duration);
    GetWorldTimerManager().SetTimer(LaunchCharacterTimer, this, &AShooterCharacter::LaunchCharacterAfterPickup, no_gravity_duration + 0.025f);
}

void AShooterCharacter::OnDebugSavePos()
{
    UE_LOG(LogTemp, Warning, TEXT("AShooterCharacter::OnDebugSavePos"));
    DebugSavePos = GetActorLocation();
}

void AShooterCharacter::OnDebugPastePos()
{
    UE_LOG(LogTemp, Warning, TEXT("AShooterCharacter::OnDebugPastePos"));
    SetActorLocation(DebugSavePos + FVector::UpVector * 10.f);
}

void AShooterCharacter::LaunchCharacterAfterPickup()
{
    LaunchCharacter(FVector::UpVector * fJumpVelocityAfterDashPickpu, true, true);
}
...
bool AShooterCharacter::IsWallRunning() const
{
    return WallRunComponent->IsWallRunning();
}

bool AShooterCharacter::IsClimbing() const
{
    return ClimbComponent->IsClimbing();
}

bool AShooterCharacter::IsDashing() const
{
    return DashComponent->IsDashing();
}

void AShooterCharacter::OnClimbStart()
{
    DashComponent->CancelDash();
}
void AShooterCharacter::OnLandAssistStart()
{
    if (IsDashing())
    {
        DashComponent->DisableDashVeloctity();
    }
}

void AShooterCharacter::ClampHorizonatlVelocity()
{
    if (!GetCharacterMovement()->IsFalling())
        return;
    ...
    if (IsDashing())
        return;
    FVector2D horizontal_vel = GetHorizontalVelocity();

    if (horizontal_vel.Size() > GetCharacterMovement()->GetMaxSpeed())
    {
        SetHorizontalVelocity(horizontal_vel.GetSafeNormal() * GetCharacterMovement()->GetMaxSpeed());
    }
}

FVector2D AShooterCharacter::GetHorizontalVelocity() const
{
    return FVector2D(GetCharacterMovement()->Velocity.X, GetCharacterMovement()->Velocity.Y);
}

void AShooterCharacter::SetHorizontalVelocity(FVector2D velocity)
{
